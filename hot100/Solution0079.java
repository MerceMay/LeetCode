...
public class Solution0079 {
    ...
    private boolean backtrack(char[][] board, String word, int index, int i, int j) {
        ...
        char temp = board[i][j];
        board[i][j] = '#';
        boolean found = backtrack(board, word, index + 1, i - 1, j) || backtrack(board, word, index + 1, i + 1, j)
                || backtrack(board, word, index + 1, i, j - 1) || backtrack(board, word, index + 1, i, j + 1);
        board[i][j] = temp;
        return found;
    }
